...
def main():
    ...
    first.create_continuous_function(log_func, -10, 20)
    second.create_continuous_function(linear_func, -10, 20)

    plot = MyPlot()
    move_axis()
    plot.add_function(first)
    plot.add_function(second)
    plot.config_plot('Примерное решение', '', '')
    plot.draw_all()
    MyPlot.show_all()  
