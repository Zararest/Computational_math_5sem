...
def straight(A, f, alpha, k_alpha, k_beta):
  ...
  for l in reversed(range(alpha + 2, size - 1)):
    ...
    B[l][l - 1] = a
    f_B[l] = b
  u_alpha = (k_alpha * f_B[alpha - 1] + k_beta * f_B[alpha + 2]) / \
            (k_alpha * (1 - B[alpha - 1][(alpha - 1) + 1]) + \
             k_beta * (1 - B[(alpha + 1) + 1][(alpha + 1) - 1]))
  print('u alpha', u_alpha, ' = ', k_alpha * f_B[alpha - 1] + k_beta * f_B[alpha + 2], ' / ', (k_alpha * (1 - B[alpha - 1][(alpha - 1) + 1]) + \
             k_beta * (1 - B[(alpha + 1) + 1][(alpha + 1) - 1])))
  B[alpha][alpha] = 1
  B[alpha + 1][alpha + 1] = 1
  f_B[alpha] = u_alpha
...
def back(A, f, alpha):
  ...
  u[alpha] = f[alpha]
  u[alpha + 1] = f[alpha + 1]
  for l in reversed(range(2, alpha)):
    a = A[l][l + 1]
    u[l] = -a * u[l + 1] + f[l]
  for l in range(alpha + 2, size - 2): 
    a = A[l][l - 1]
    u[l] = -a * u[l - 1] + f[l]
  return u
